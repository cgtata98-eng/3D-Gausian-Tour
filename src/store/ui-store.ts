...
interface UIState {
  /**
   * `?mode=dev`. Also the switch for features that are still being built:
   * they must be invisible everywhere else in BOTH senses — no control on
   * screen AND no behaviour behind it, because gating only the button leaves
   * the keys, the auto-entry and the fallbacks running, which is how a
   * half-shipped feature reaches a customer. Currently gates the dense
   * walkthrough; see `WALKTHROUGH_AUTHORING_ONLY`.
   */
  isDeveloper: boolean;
