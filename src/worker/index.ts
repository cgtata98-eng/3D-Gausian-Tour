...
import { handleAiEdit } from '../shared/ai-edit-handler';
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === '/api/ai/edit') {
      // Shared with the dev Vite proxy (src/shared/ai-edit-handler.ts) so the
      // guards / error shapes can't drift between dev and production.
      return handleAiEdit(request, { gemini: env.GEMINI_API_KEY, openai: env.OPENAI_API_KEY });
    }
