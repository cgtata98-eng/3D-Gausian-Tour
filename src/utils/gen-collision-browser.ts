...
export async function generateCollisionInBrowser(
  ...
  // --voxel-external-fill 1.6) so dev and prod produce the same geometry.
  ...
  const recipeOpts = opts.recipe === 'walkable'
    ...
    : {
        navSeed,
        // 0.3 m (≈2 voxels at 0.15 m) — large enough to seal sub-decimeter
        // scan gaps but small enough not to punch through thin walls and
        // collapse the navigable interior. Default 1.6 dilates ~11 voxels
